...
def parse_tweet(users, source_sites, keywords, source_accounts):
    ...
    for account in users:
        ...
        processed = 0
        # get user info
        twarc_user = next(auth.user_lookup([user], "screen_name"))
        ...
        for ht in hashtags:
            print(("{}: {}".format(ht, hashtags[ht])))
            logging.info("{}: {}".format(ht, hashtags[ht]))
        ...
        for mentioned_user, count in list(mentions.items()):
            print(("{}: {}".format(mentioned_user, count)))
            logging.info("{}: {}".format(mentioned_user, count))
