...
def articles_keywords_pie_chart():
    ...
    for ele in keywords:
        ...
        if not ele.name in list(data_dict.keys()):
    ...
    sorted_data = sorted(list(data_dict.items()), key=operator.itemgetter(1), reverse=True)
    ...
    for ele in list(sorted_data_dict.keys()):
        new=[]
