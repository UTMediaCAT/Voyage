...
import sys, os, time, json, yaml, urllib.request, urllib.parse, urllib.error
import common
import subprocess
...
def getJson(request):
    ...
    for key in Keyword.objects.all():
        articles[key.version.article.url]['matched_keywords'].append(key.name)
    for src in SourceSite.objects.all():
        articles[src.version.article.url]['matched_source_sites'].append({'url':src.url,
                                                             'site': src.domain, 'matched': src.matched})
    for src in SourceTwitter.objects.all():
        articles[src.version.article.url]['matched_source_twitter_accounts'].append({'name':src.name,
                                                             'matched': src.matched})
    for ath in Author.objects.all():
        articles[ath.version.article.url]['authors'].append(ath.name)
...
def getWarc(request, filename):
    try:
        ...
        if 'WebKit' in request.META['HTTP_USER_AGENT']:
            ...
            filename_header = 'filename*=UTF-8\'\'%s' % urllib.parse.quote((filename + ".warc.gz").encode('utf-8'))
